  ...
  const publish = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      const payloadResult = createAuctionSchema.safeParse({
        ...
        avg_weight_kg:
          Number.isFinite(values.avg_weight_kg) && Number(values.avg_weight_kg) > 0
      ...
      if (!payloadResult.success) {
        throw new Error(payloadResult.error.issues[0]?.message ?? "Invalid auction details");
      }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Publish premium listings with compliant animal and permit metadata.</CardDescription>
      ...
      <CardContent className="space-y-6">
        {step === 0 ? (
          <div className="grid gap-4">
            <div className="space-y-2">
              ...
              <Input id="title" placeholder="Prime Kalahari Buffalo Bull" {...form.register("title")} />
            </div>
            <div className="space-y-2">
        ...
        {step === 1 ? (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="number" min={1} step="1" {...form.register("animal_count", { valueAsNumber: true })} />
            </div>
            <div className="space-y-2">
              <Label>Average weight (kg)</Label>
              <Input type="number" min={1} step="0.1" {...form.register("avg_weight_kg", { valueAsNumber: true })} />
              <p className="text-xs text-slate-500">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => form.setValue("sex", value === "none" ? "" : value, { shouldValidate: true })}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select value={form.watch("province")} onValueChange={(value) => form.setValue("province", value)}>
                <SelectTrigger>
        ...
        {step === 2 ? (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="number" step="1" min={1} {...form.register("starting_bid", { valueAsNumber: true })} />
            </div>
            <div className="space-y-2">
              <Label>Minimum increment (ZAR)</Label>
              <Input type="number" step="1" min={1} {...form.register("min_increment", { valueAsNumber: true })} />
            </div>
            <div className="space-y-2 sm:col-span-2">
              <Label>Reserve price (optional)</Label>
              <Input type="number" step="1" min={1} {...form.register("reserve_price", { valueAsNumber: true })} />
            </div>
            <div className="space-y-2 sm:col-span-2">
        ...
        {step === 3 ? (
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input type="datetime-local" {...form.register("start_time_local")} />
            </div>
            <div className="space-y-2">
              <Label>End time (Johannesburg)</Label>
              <Input type="datetime-local" {...form.register("end_time_local")} />
            </div>
          </div>
        ) : null}
        ...
        <div className="flex items-center justify-between">
          ...
          {step < stepTitles.length - 1 ? (
            <Button type="button" onClick={() => setStep((prev) => Math.min(stepTitles.length - 1, prev + 1))}>
              Continue
            </Button>
          ) : (
            <Button type="button" onClick={publish} disabled={publishing}>
              {publishing ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : null}
              Publish auction
            </Button>
          )}
        </div>
